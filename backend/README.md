...
Payment gateways have been removed to rebuild the payment flow from scratch.
Checkout currently only places the order (reserves inventory, creates a PLACED
order). The `Payment` model and the order/refund/cancellation/settlement
plumbing remain in place; wire a new gateway into `PaymentService` and add its
routes when integrating.
