...
async function main() {
    ...
    const taken = new Set((await prisma.product.findMany({
        where: { slug: { not: null } },
        select: { slug: true },
    })).map((row) => row.slug));
    console.log(`[backfill-product-slugs] mode=${apply ? 'APPLY' : 'DRY-RUN'} missing=${products.length}`);
    let written = 0;
    ...
    console.log(`[backfill-product-slugs] complete mode=${apply ? 'APPLY' : 'DRY-RUN'} slugs=${written}`);
}
main()
    .catch((error) => {
...
//# sourceMappingURL=backfill-product-slugs.js.map