...
async function verifyOrders() {
    ...
    const variant = productDetail.data.product?.variants?.find((v) => v.id === variantId);
    ...
    if (ordersRes.status === 200 && ordersRes.data.orders?.length > 0) {
        const foundOrder = ordersRes.data.orders.find((o) => o.id === orderId);
        if (foundOrder) {
    ...
    if (sellerOrdersRes.status === 200) {
        const sellerItems = sellerOrdersRes.data.orderItems?.filter((item) => item.order?.id === orderId);
