 ...
 */
configRouter.get('/checkout', (req, res, next) => fastrrController.getCheckoutConfig(req, res, next));
/**
 * GET /v1/config/shipping
 * Current shipping-charge configuration for display in cart/checkout.
 */
configRouter.get('/shipping', async (_req, res, next) => {
    try {
 ...
 */
configRouter.get('/gst', async (_req, res, next) => {
    try {
