 ...
 */
function warnIfDltMissing() {
    if (hasDltConfig() || env.AQUASMS_ALLOW_NON_DLT || warnedAboutMissingDlt)
        return;
    warnedAboutMissingDlt = true;
    logger.warn({ event: 'sms_dlt_not_configured' }, 'AQUASMS_DLT_PE_ID / AQUASMS_DLT_TEMPLATE_ID are not set. Messages will be ' +
 ...
 */
function extractMessageId(rows) {
...
function interpretResponse(raw) {
    ...
    const errorRow = rows.find((row) => typeof row.responseCode === 'string' && !SUCCESS_HINT.test(row.responseCode));
    if (errorRow) {
        return { ok: false, reason: String(errorRow.responseCode) };
    }
    const messageId = extractMessageId(rows);
    const hasSuccessCode = rows.some((row) => typeof row.responseCode === 'string' && SUCCESS_HINT.test(row.responseCode));
...
class AquaSmsService {
    ...
    async sendSms(phone, message) {
        ...
        try {
            ...
            if (!response.ok) {
                throw ApiError.internal(`SMS provider returned HTTP ${response.status}: ${raw.slice(0, 200)}`);
            }
        }
        catch (err) {
        ...
        logger.info({ event: 'sms_sent', phone: numbers, dlt: useDlt, messageId: result.messageId }, 'SMS sent');
