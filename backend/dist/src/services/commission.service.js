...
import { sellerSettlementCreatedTotal, sellerSettlementAmountTotal, commissionAmountTotal, } from '../config/metrics.js';
...
class CommissionService {
     ...
     */
    async calculateAndStoreSellerSettlement(orderId) {
        ...
        if (!order.payment || order.payment.status !== PaymentStatus.SUCCESS) {
            commissionLogger.warn({ orderId, paymentStatus: order.payment?.status }, 'Skipping settlement — payment not SUCCESS');
            return;
        }
