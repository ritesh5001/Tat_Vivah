...
export class RefundService {
    ...
    async createRefund(input) {
        ...
        if (existingSuccess) {
            refundLogger.info({ orderId, refundId: existingSuccess.id, amount }, 'refund_idempotent_hit');
        ...
        const refund = await prisma.$transaction(async (tx) => {
            ...
            if (alreadyRefundedPaise + amount > orderTotalPaise) {
                ...
                refundLogger.warn({ orderId, requestedAmount: amount, alreadyRefunded: alreadyRefundedPaise, orderTotal: orderTotalPaise }, 'refund_over_limit_rejected');
                throw ApiError.badRequest(`Refund exceeds order total. Already refunded: ${alreadyRefundedPaise} paise, requested: ${amount} paise, limit: ${orderTotalPaise} paise`);
            }
        ...
        if (providerSuccess) {
            ...
            if (updated.count > 0) {
                ...
                refundLogger.info({ orderId, refundId: refund.id, razorpayRefundId, amount }, 'refund_success');
            ...
            refundLogger.error({ orderId, refundId: refund.id, amount }, 'refund_failed');
