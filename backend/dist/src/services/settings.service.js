...
export const settingsService = {
    ...
    async setShippingChargeEnabled(enabled) {
        await writeRaw(SETTING_KEYS.SHIPPING_CHARGE_ENABLED, enabled ? 'true' : 'false');
        settingsLogger.info({ event: 'shipping_charge_toggled', enabled }, `Shipping charge ${enabled ? 'enabled' : 'disabled'} by admin`);
        return enabled;
    },
    /**
    ...
    async setGstChargeEnabled(enabled) {
        await writeRaw(SETTING_KEYS.GST_CHARGE_ENABLED, enabled ? 'true' : 'false');
        settingsLogger.info({ event: 'gst_charge_toggled', enabled }, `Flat GST charge ${enabled ? 'enabled' : 'disabled'} by admin`);
        return enabled;
    },
};
