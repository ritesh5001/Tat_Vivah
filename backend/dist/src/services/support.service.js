...
import { publishLiveDashboardEvent } from '../live/live-events.js';
const ADMIN_ROLES = ['ADMIN', 'SUPER_ADMIN'];
const DEFAULT_TICKET_LIMIT = 20;
const DEFAULT_MESSAGE_LIMIT = 50;
const PREVIEW_LENGTH = 140;
export const isSupportAdmin = (role) => ADMIN_ROLES.includes(role);
const toPreview = (body) => body.length > PREVIEW_LENGTH ? `${body.slice(0, PREVIEW_LENGTH - 1)}…` : body;
...
const toParticipant = (user) => {
    ...
    return {
        ...
        email: user.email,
        phone: user.phone,
        name: user.seller_profiles?.store_name ??
...
const toTicketResponse = (ticket, viewerIsAdmin) => ({
    id: ticket.id,
...
export class SupportService {
    ...
    async loadTicketForViewer(ticketId, viewerId, viewerRole) {
    ...
    async createTicket(requesterId, requesterRole, input) {
    ...
    async listTickets(viewerId, viewerRole, query) {
    ...
    async getTicket(ticketId, viewerId, viewerRole) {
     ...
     */
    async listMessages(ticketId, viewerId, viewerRole, query) {
        const ticket = await this.loadTicketForViewer(ticketId, viewerId, viewerRole);
    ...
    async sendMessage(ticketId, senderId, senderRole, input) {
    ...
    async markRead(ticketId, viewerId, viewerRole) {
        const ticket = await this.loadTicketForViewer(ticketId, viewerId, viewerRole);
        const viewerIsAdmin = isSupportAdmin(viewerRole);
    ...
    async updateTicket(ticketId, actorId, actorRole, input) {
    ...
    async unreadSummary(viewerId, viewerRole) {
        const viewerIsAdmin = isSupportAdmin(viewerRole);
