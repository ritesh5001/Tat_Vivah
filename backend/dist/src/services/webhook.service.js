...
export class WebhookService {
    ...
    // ------------------------------------------------------------------
    async handlePhonePeWebhook(payload, authorizationHeader) {
        ...
        paymentLogger.info({ event: 'phonepe_webhook_received', webhookEvent: event.event, merchantOrderId: event.merchantOrderId }, `PhonePe webhook received: ${event.event}`);
        if (!event.event.startsWith('checkout.order.')) {
            paymentLogger.info({ event: 'phonepe_webhook_unhandled', webhookEvent: event.event }, `PhonePe webhook unhandled event: ${event.event}`);
            return;
        }
        ...
        if (!payment) {
            ...
            paymentLogger.warn({ event: 'phonepe_webhook_payment_not_found', merchantOrderId: event.merchantOrderId }, `PhonePe webhook: no local payment for ${event.merchantOrderId} — skipping`);
            return;
        }
        ...
        await prisma.paymentEvent.create({
            data: { paymentId: payment.id, type: PaymentEventType.WEBHOOK, payload: payload },
        });
        if (event.event === 'checkout.order.completed' && event.state === 'COMPLETED') {
            ...
            if (status.state !== 'COMPLETED') {
                paymentLogger.warn({ event: 'phonepe_webhook_state_mismatch', merchantOrderId: event.merchantOrderId, state: status.state }, 'PhonePe webhook: status API disagrees with webhook, skipping');
                return;
            }
            const transactionId = status.paymentDetails?.[0]?.transactionId ?? event.transactionId ?? status.orderId;
            await paymentService.handlePaymentSuccess(payment.id, payment.orderId, transactionId, { merchantOrderId: event.merchantOrderId, phonepeOrderId: status.orderId, source: 'webhook' });
            return;
        }
