...
async function createUser(suffix: number): Promise<{ id: string; token: string }> {
...
async function main() {
    ...
    if (!categoryId) throw new Error('No categories found — seed the DB first');
    ...
    if (!productId) throw new Error(`Failed to create product: ${JSON.stringify(prodRes.data)}`);
    ...
    if (!variantId) throw new Error(`Failed to create variant: ${JSON.stringify(varRes.data)}`);
    ...
    const buyers: { id: string; token: string }[] = [];
    ...
    const results = await Promise.allSettled(
        buyers.map((buyer) =>
            api('/v1/checkout', 'POST', {}, buyer.token)
        )
    );
    ...
    assert(
        `Exactly ${STOCK_LIMIT} succeed`,
        successes === STOCK_LIMIT,
        `expected ${STOCK_LIMIT}, got ${successes}`
    );
    assert(
        `Exactly ${CONCURRENT_USERS - STOCK_LIMIT} conflict (409)`,
        conflicts === CONCURRENT_USERS - STOCK_LIMIT,
        `expected ${CONCURRENT_USERS - STOCK_LIMIT}, got ${conflicts}`
    );
    assert(
        'Final stock === 0',
        finalStock === 0,
        `expected 0, got ${finalStock}`
    );
    assert(
        'Stock never negative',
        finalStock >= 0,
        `stock went to ${finalStock}`
    );
    assert(
        `RESERVE movements === ${STOCK_LIMIT}`,
        reserveCount === STOCK_LIMIT,
        `expected ${STOCK_LIMIT}, got ${reserveCount}`
    );
    assert(
        'RELEASE movements === 0 (no cancellations yet)',
        releaseCount === 0,
        `expected 0, got ${releaseCount}`
    );
    assert(
        `Orders created === ${STOCK_LIMIT}`,
        ordersCreated === STOCK_LIMIT,
        `expected ${STOCK_LIMIT}, got ${ordersCreated}`
    );
    assert(
        'Total movements match orders',
        reserveCount === ordersCreated,
        `RESERVE=${reserveCount} vs orders=${ordersCreated}`
    );
    assert(
        'No unexpected errors',
        otherErrors === 0,
        `got ${otherErrors} unexpected errors`
    );
    assert(
        `Success rate is exactly ${((STOCK_LIMIT / CONCURRENT_USERS) * 100).toFixed(1)}%`,
        successes === STOCK_LIMIT,
        `got ${successRate}%`
    );
