...
async function run(): Promise<void> {
    ...
    const returnReq = await returnService.requestReturn(
        buyer.id,
        order.id,
        'Race simulation return request',
    ...
    if (finalReturn?.status === ReturnStatus.REFUNDED) {
        assert(
            finalOrder?.payment?.status === PaymentStatus.REFUNDED,
            'Payment should be REFUNDED when return is REFUNDED',
        );
    }
    ...
    const approveOutcomes = results.slice(0, 2).map((r) => {
        if (r.status === 'fulfilled') return (r.value as any).alreadyApproved ? 'idempotent' : 'first';
        return 'rejected';
    });
