...
async function seedUser(suffix: string, role: Role = Role.USER) {
...
async function seedProductAndInventory(sellerId: string, stock: number) {
...
async function testConcurrentRefundPrevention() {
    ...
    assert(
        totalRefundedPaise <= orderTotalPaise,
        `Over-refund prevented: refunded ${totalRefundedPaise} paise <= limit ${orderTotalPaise} paise (${successes.length} succeeded, ${failures.length} rejected)`,
    );
}
...
async function testConcurrentCancellationRequests() {
    ...
    const results = await Promise.allSettled(
        Array.from({ length: 5 }, () =>
            cancellationService.requestCancellation(buyer.id, order.id, 'concurrent test'),
        ),
    );
    ...
    assert(
        ...
        `Exactly 1 cancellation request exists (not ${cancellations.length}): ${successes.length} succeeded, ${failures.length} rejected`,
    );
}
...
async function testInventoryMovementReasonDiscriminator() {
    ...
    assert(
        movements.length === 2,
        `Two separate RELEASE movements (CANCELLATION + RETURN) coexist for same order+variant`,
    );
}
...
async function testRefundLedgerConsistency() {
    ...
    for (const order of orders) {
        ...
        if (totalRefundedPaise > orderTotalPaise) {
            violations++;
            LOG.error(
                `Order ${order.id}: refunded ${totalRefundedPaise} paise > total ${orderTotalPaise} paise`,
            );
        }
    }
...
async function testInventoryIntegrity() {
    ...
    assert(
        ...
        `No negative stock found (checked all inventory rows)`,
    );
}
