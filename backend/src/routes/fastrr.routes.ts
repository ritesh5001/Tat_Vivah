...
fastrrRouter.post('/checkout/token', (req, res, next) =>
    fastrrController.createToken(req, res, next),
);

/** GET /v1/fastrr/checkout/sessions/:sessionId — how did it end? */
fastrrRouter.get('/checkout/sessions/:sessionId', (req, res, next) =>
    fastrrController.getSessionStatus(req, res, next),
);
