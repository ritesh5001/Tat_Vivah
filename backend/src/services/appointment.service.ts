...
const ACTIVE_BOOKED_STATUSES: AppointmentStatus[] = ['PENDING', 'CONFIRMED'];
...
function getAppointmentEnd(date: Date, time: string): Date {
...
export class AppointmentService {
  ...
  private async assertBuyer(userId: string) {
  ...
  async listUserAppointments(userId: string) {
    await this.autoCompletePastAppointments();
    await this.assertBuyer(userId);
  ...
  async listSellerAppointments(sellerId: string) {
    await this.autoCompletePastAppointments();
  ...
  async updateAppointmentStatus(actor: ActorContext, input: UpdateAppointmentStatusInput) {
    await this.autoCompletePastAppointments();
  ...
  async rescheduleAppointment(actor: ActorContext, input: RescheduleAppointmentInput) {
  ...
  async blockSeller(actor: ActorContext, input: BlockSellerInput) {
    if (!['ADMIN', 'SUPER_ADMIN'].includes(actor.role)) {
  ...
  async listSellerAvailability(sellerId: string) {
    await this.assertSeller(sellerId);
  ...
  async upsertSellerAvailability(sellerId: string, input: UpsertSellerAvailabilityInput) {
    await this.assertSeller(sellerId);
