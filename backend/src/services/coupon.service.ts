...
export class CouponService {
    ...
    private async withTx<T>(tx: TxClient | undefined, fn: (trx: TxClient) => Promise<T>): Promise<T> {
        if (tx) return fn(tx);
        return prisma.$transaction((trx) => fn(trx));
    }

    async applyCouponToOrder(input: ApplyCouponInput): Promise<{
        couponId: string;
        couponCode: string;
        discountAmount: Prisma.Decimal;
    }> {
    ...
    async redeemCouponAfterOrderCreated(input: RedeemCouponInput): Promise<void> {
    ...
    async validateCouponCode(userId: string, code: string): Promise<{
        valid: boolean;
        message?: string;
        coupon?: {
            code: string;
