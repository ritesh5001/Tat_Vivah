...
export class CouponAdminService {
    ...
    async updateCoupon(id: string, input: UpdateCouponInput) {
        ...
        if (input.type !== undefined) data.type = input.type as CouponType;
        ...
        if (input.firstTimeUserOnly !== undefined) data.firstTimeUserOnly = input.firstTimeUserOnly;
    ...
    async deleteCoupon(id: string) {
        ...
        if (coupon._count.redemptions > 0) {
            throw ApiError.badRequest(
                `Cannot delete coupon with ${coupon._count.redemptions} existing redemption(s). Deactivate it instead.`
            );
        }
