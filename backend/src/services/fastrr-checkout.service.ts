...
import {
    createFastrrAccessToken,
    getFastrrUiAssets,
    isFastrrCheckoutEnabled,
    type FastrrCartItem,
...
/** A validated line, carrying everything both Fastrr and the later order need. */
...
export class FastrrCheckoutService {
     ...
     */
    async createSession(request: FastrrTokenRequest): Promise<FastrrTokenResponse> {
        if (!isFastrrCheckoutEnabled()) {
            throw new ApiError(503, 'Express checkout is currently unavailable');
        }

        const { cartId, lines } = await this.validateSelection(
            request.userId,
            request.variantIds,
        );
        ...
        const items: FastrrCartItem[] = lines.map((line) => ({
    ...
    private async validateSelection(
        userId: string,
        variantIds: string[] | undefined,
    ): Promise<{ cartId: string | null; lines: SelectedLine[] }> {
        ...
        if (selected.length > MAX_CHECKOUT_ITEMS) {
            throw ApiError.badRequest(
                `Cart cannot contain more than ${MAX_CHECKOUT_ITEMS} items per checkout`,
            );
        }
        ...
        for (const row of selected) {
            ...
            if (row.sellerId == null || row.variantStatus == null || row.variantPrice == null) {
                ...
                errors.push(
                    `Insufficient stock for ${title}: Available ${row.stock}, Requested ${row.quantity}`,
                );
            }
        }
        ...
        const lines: SelectedLine[] = selected.map((row) => {
            ...
            if (!variant?.externalId) {
                throw new ApiError(
                    409,
                    `${row.productTitle ?? 'An item'} is not available for express checkout yet. Please use standard checkout.`,
                );
            }
     ...
     */
    private buildRedirectUrl(sessionId: string, mobileApp: boolean): string {
        const base = this.frontendBaseUrl();
        const params = new URLSearchParams({ sid: sessionId });
        if (mobileApp) params.set('app', '1');
