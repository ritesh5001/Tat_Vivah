 ...
 * Invoice Service
 *
 * Generates GST-compliant PDF invoices from stored order snapshots.
 * NEVER recalculates tax — uses only the values persisted at checkout time.
 */
...
export async function generateInvoicePDF(orderId: string): Promise<Buffer> {
    ...
    doc.fontSize(9).font('Helvetica');
    doc.text(buyerName);
    if (order.shippingAddressLine1) doc.text(order.shippingAddressLine1);
    if (order.shippingAddressLine2) doc.text(order.shippingAddressLine2);
    if (order.shippingCity) doc.text(order.shippingCity);
    if (buyerState) doc.text(`State: ${buyerState}`);
    doc.moveDown(1);

    // ======== SELLER DETAILS ========
    const sellers = new Map<string, { name: string; gstin: string; state: string }>();
    for (const item of order.items) {
        const sp = (item.product as any)?.seller?.seller_profiles;
    ...
    const shippingAmount = Math.max(
        order.grandTotal - order.subTotalAmount - order.totalTaxAmount,
        0
    );
 ...
 */
export function recordInvoiceDownload(): void {
...
function drawTotalLine(doc: PDFKit.PDFDocument, label: string, value: string, x: number): void {
    const y = doc.y;
