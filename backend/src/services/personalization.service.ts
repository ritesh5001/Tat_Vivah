...
import {
    recentlyViewedTrackTotal,
    recentlyViewedFetchTotal,
...
export class PersonalizationService {
     ...
     */
    async getRecentlyViewed(userId: string): Promise<RecentlyViewedProduct[]> {
        ...
        const raw = await redis.zrange<string[]>(key, 0, MAX_ITEMS - 1, { rev: true, withScores: true });
        ...
        const result: RecentlyViewedProduct[] = [];
