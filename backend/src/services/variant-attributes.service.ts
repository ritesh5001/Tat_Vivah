 ...
 */
const COLOR_PHRASES = [...new Set(COLOR_VOCABULARY)].sort(
    (a, b) => b.split(' ').length - a.split(' ').length || b.length - a.length
);

/** Folds a vendor spelling onto its library name. */
export function canonicalizeColor(name: string): string {
...
export function titleCaseColor(value: string): string {
 ...
 */
export function resolveColorHex(name?: string | null): string | null {
    ...
    for (let start = 0; start < words.length; start += 1) {
        ...
        if (modifiers.includes('light') || modifiers.includes('pale')) return shade(base, 0.35);
        if (modifiers.includes('dark') || modifiers.includes('deep')) return shade(base, -0.3);
        return base;
    }
 ...
 */
export function extractColorPhrase(text?: string | null): string | null {
    ...
    for (let end = words.length; end > 0; end -= 1) {
        for (const phrase of COLOR_PHRASES) {
            ...
            if (start < 0) continue;
            if (!parts.every((part, index) => words[start + index] === part)) continue;
            ...
            const picked = words.slice(from, end).filter((word, index, all) => {
                if (word !== 'light' && word !== 'dark') return true;
 ...
 */
function splitGluedColors(word: string): string[] {
 ...
 */
export function parseSizeLabel(raw?: string | null): ParsedSize {
    const trimmed = (raw ?? '').trim();
    if (!trimmed) return { size: 'Free Size', colorPrefix: null };
 ...
 */
export function normalizeVariantAttributes(input: VariantAttributeInput): NormalizedVariantAttributes {
    const result: NormalizedVariantAttributes = {
        size: undefined,
        color: undefined,
        derivedColorHex: null,
    };

    let liftedColor: string | null = null;
...
export function skuColorSegment(color?: string | null): string {
 ...
 */
export function buildSku(base: string, color: string | null, size: string): string {
