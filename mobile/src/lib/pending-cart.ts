 ...
 * the buyer stared at the product screen for the whole request before anything moved.
 * Now the navigation happens immediately and the request keeps running; the app is a
 * single JS context, so this module's state survives the screen change and checkout
 * can wait for the write at the point where it actually matters:
 *
 *   - before placing the order, so the server cart is guaranteed to contain the item
 *
 * The cart store already updates optimistically, so the screen itself renders the
 * item instantly; this only closes the window where Place Order could beat the
 * add-to-cart request and fail with "Cart is empty".
 */
