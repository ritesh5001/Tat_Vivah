/**
 * Rules for presenting variant attributes, mirroring the web storefront's
 * `frontend/src/lib/variant-attributes.ts`.
 *
 * The backend normalises what is *stored*; this decides how the result is
 * ordered and grouped, so a size row reads "36 38 40 42 44" / "S M L XL"
 * instead of whatever order the rows came back in.
 */
...
export interface SizeOption<TVariant> {
  ...
  stock: number | null;
  inStock: boolean;
  /** Set when stock is low enough to be worth nudging the shopper with. */
 ...
 *
 * Stock only counts as sold out when the API actually reported a count — a
 * missing `inventory` means "not tracked", and disabling every chip would be
 * worse than letting the bag reject it.
 */
